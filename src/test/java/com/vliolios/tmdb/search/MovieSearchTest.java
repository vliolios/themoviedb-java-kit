...
public class MovieSearchTest extends WireMockTest {
	...
	public void testSubmitResponseSuccessful() {
		...
		Response<MovieResult> response = MovieSearch.apiConfig(apiConfig).query("matrix").page(0).language("en").includeAdult(true).region("US").year(2000).primaryReleaseYear(1990).build().submit();
	...
	public void testSubmitResponseInvalid() {
		...
		Response<MovieResult> response = MovieSearch.apiConfig(apiConfig).query("matrix").build().submit();
	...
	public void testQuery() {
		MovieSearch search = MovieSearch.apiConfig(apiConfig).query("matrix").build();
		assertThat("The query is incorrect", search.getQuery(), is("matrix"));
	}
	
	@Test
	public void testPage() {
		MovieSearch search = MovieSearch.apiConfig(apiConfig).query("matrix").page(1).build();
		assertThat("The page is incorrect", search.getPage(), is(1));
	}
	
	@Test
	public void testLanguage() {
		MovieSearch search = MovieSearch.apiConfig(apiConfig).query("matrix").language("en").build();
		assertThat("The language is incorrect", search.getLanguage(), is("en"));
	}

	@Test
	public void testIncludeAdult() {
		MovieSearch search = MovieSearch.apiConfig(apiConfig).query("matrix").includeAdult(true).build();
		assertThat("The include adult flag is incorrect", search.getIncludeAdult(), is(true));
	}

	@Test
	public void testRegion() {
		MovieSearch search = MovieSearch.apiConfig(apiConfig).query("matrix").region("US").build();
		assertThat("The region is incorrect", search.getRegion(), is("US"));
	}

	@Test
	public void testYear() {
		MovieSearch search = MovieSearch.apiConfig(apiConfig).query("matrix").year(2000).build();
		assertThat("The year is incorrect", search.getYear(), is(2000));
	}

	@Test
	public void testPrimaryReleaseYear() {
		MovieSearch search = MovieSearch.apiConfig(apiConfig).query("matrix").primaryReleaseYear(2000).build();
